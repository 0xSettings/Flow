...
impl<T: Chunk> Node<T> {
    ...
    pub fn id(&self) -> uuid::Uuid {
        // Cross-platform: use a lossy UTF-8 representation of the path
        let path_str = self.path.to_string_lossy();

        // Pre-allocate buffer for path + chunk
        let mut bytes = Vec::with_capacity(path_str.len() + self.chunk.as_ref().len());
        bytes.extend_from_slice(path_str.as_bytes());
        bytes.extend_from_slice(self.chunk.as_ref());
